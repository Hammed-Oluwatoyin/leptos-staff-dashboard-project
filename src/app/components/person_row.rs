...
pub fn PersonRow(
    ...
    let (if_show_info_modal, set_if_show_info_modal) = signal(false);
    let (if_show_edit_modal, set_if_show_edit_modal) = signal(false);
    ...
    view! {
        ...
        <div class=ROW_STYLE>
            <div class="flex flex-col w-full max-w-[45rem]">
                ...
                <p class="text-sm text-stone-400">{person.title.clone()}</p>
            </div>
